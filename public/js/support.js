...
window.addEventListener('load', function () {
    window.verifySession(['support', 'administrador', 'gerencia'], function (user) {
        currentUser = user;
        loadMyTickets();
    ...
    fetch('/api/login', {
        ...
        .then(data => {
            if (data.token && data.user) {
                if (data.user.role !== 'support' && data.user.role !== 'gerencia') {
                    errorDiv.textContent = 'Acceso denegado. Solo personal de soporte puede acceder.';
                    return;
                }
    ...
    try {
        ...
        if (helpEl) {
            helpEl.innerHTML = newTechnician 
                ? `Asignado a: <strong style="color: #008B8B;">${escapeHtml(newTechnician)}</strong>` 
...
function showTicketDetails(ticketId) {
    ...
    // --- Estructura HTML Rediseñada (Inline Styles Style) ---
    modalBody.innerHTML = `
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin: 15px 0;">
            ...
            <div class="detail-row" style="margin: 0; padding: 15px; background: #f8f9fa; border-radius: 8px;">
                ...
                <select id="ticketTechnicianSelect-${ticket.id}" 
                    ...
                    <option value="Rodolfo" ${ticket.assigned_technician === 'Rodolfo' ? 'selected' : ''}>Rodolfo</option>
                    <option value="Matias" ${ticket.assigned_technician === 'Matias' ? 'selected' : ''}>Matias</option>
                </select>
                <small id="ticketTechnicianHelp-${ticket.id}" style="display:block; margin-top:4px; font-size:0.75em; color:#6c757d;">
                    ${ticket.assigned_technician ? `Asignado a: <strong style="color: #008B8B;">${escapeHtml(ticket.assigned_technician)}</strong>` : 'Selecciona el técnico responsable'}
                </small>
            </div>
        </div>
    ...
    if (typeof updatedTicket.assigned_technician !== 'undefined') {
        ...
        if (techHelp) {
            techHelp.innerHTML = updatedTicket.assigned_technician 
                ? `Asignado a: <strong style="color: #008B8B;">${escapeHtml(updatedTicket.assigned_technician)}</strong>` 
...
// =======================================================
// TABLERO DE TAREAS & CHECKLIST - SOPORTE TÉCNICO
// =======================================================

let currentSupportTab = 'tickets';
let allSupportTasks = [];
...
async function loadSupportTasks() {
    ...
    params.append('department', 'Sistemas');
        ...
        if (container) {
            container.innerHTML = `
                <div style="grid-column: 1 / -1; text-align: center; padding: 2rem; color: #EF4444; background: white;">
                    ❌ Error al cargar tareas de soporte: ${escapeHtml(err.message)}
                    <br><button onclick="loadSupportTasks()" style="margin-top: 10px; padding: 6px 14px; background: #008B8B; color: white; border: none; border-radius: 6px; cursor: pointer;">Reintentar</button>
                </div>
            `;
        }
    }
...
async function loadSupportTaskStats() {
    try {
        const response = await fetch('/api/maintenance/tasks-stats?department=Sistemas', {
            credentials: 'include',
            headers: { 'Accept': 'application/json' }
        });
...
// =======================================================
// MOTOR DE RENDERIZADO ESTILO GOOGLE CALENDAR (SOPORTE)
// =======================================================
...
function getTasksForDay(dateKey) {
    return allSupportTasks.filter(task => {
...
function renderCalendarAgenda() {
    ...
    if (!Array.isArray(allSupportTasks) || allSupportTasks.length === 0) {
    ...
    allSupportTasks.forEach(task => {
        const key = task.due_date ? task.due_date.substring(0, 10) : 'Sin fecha límite';
    ...
    sortedKeys.forEach(dateKey => {
        ...
        if (dateKey !== 'Sin fecha límite') {
            const parts = dateKey.split('-');
            const d = new Date(parts[0], parseInt(parts[1], 10) - 1, parts[2]);
            headerLabel = d.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' });
            headerLabel = headerLabel.charAt(0).toUpperCase() + headerLabel.slice(1);
        }

        const tasksList = grouped[dateKey];
        agendaHtml += `
            <div class="agenda-group">
                <div class="agenda-group-header">
                    <span>📅 ${headerLabel}</span>
                    <span class="agenda-count-badge">${tasksList.length} ${tasksList.length === 1 ? 'tarea' : 'tareas'}</span>
                </div>
                <div class="agenda-group-cards">
                    ${tasksList.map(task => {
                        ...
                        return `
                            <div class="agenda-task-item ${priorityClass} ${isCompleted ? 'status-completed' : ''}"
                                ...
                                <div class="agenda-item-right">
                                    <button class="btn-agenda-view" onclick="event.stopPropagation(); openTaskDetailModal(${task.id});">
                                        Ver Detalle ➔
                                    </button>
                                </div>
                            </div>
...
// =======================================================
// MODAL DE CREACIÓN / EDICIÓN DE TAREA (SOPORTE)
// =======================================================
...
function openTaskModal(task = null, prefillDate = null) {
    ...
    document.getElementById('recurrenceOptionsDiv').style.display = 'none';
    document.getElementById('taskIsRecurringInput').checked = false;

    if (task) {
        titleHeader.textContent = '✏️ Editar Tarea de Soporte';
        ...
        document.getElementById('taskSedeInput').value = task.sede || 'Todas';
        const assignedInput = document.getElementById('taskAssignedToInput');
        if (assignedInput) assignedInput.value = task.assigned_technician || '';
        document.getElementById('taskDueDateInput').value = task.due_date ? task.due_date.substring(0, 10) : '';
        ...
        if (task.is_recurring) {
            ...
            document.getElementById('taskIsRecurringInput').checked = false;
            document.getElementById('recurrenceOptionsDiv').style.display = 'none';
            document.getElementById('taskRecurrenceIntervalInput').value = 'weekly';
            selectedRecurrenceDays = [];
            document.getElementById('recurrenceCustomDaysDiv').style.display = 'none';
            renderRecurrenceDayPills();
        }
        ...
        if (Array.isArray(task.checklist)) {
            taskChecklistBuilderItems = [...task.checklist];
        }
    } else {
        titleHeader.textContent = '➕ Nueva Tarea de Soporte Técnico';
        const assignedInput = document.getElementById('taskAssignedToInput');
        if (assignedInput) assignedInput.value = '';
        if (prefillDate) {
            document.getElementById('taskDueDateInput').value = prefillDate;
        } else {
            document.getElementById('taskDueDateInput').value = formatDateKey(new Date());
        }
        document.getElementById('taskIsRecurringInput').checked = false;
        document.getElementById('recurrenceOptionsDiv').style.display = 'none';
        document.getElementById('taskRecurrenceIntervalInput').value = 'weekly';
...
function removeChecklistItemBuilder(idx) {
    taskChecklistBuilderItems.splice(idx, 1);
    renderChecklistBuilder();
}
...
function renderChecklistBuilder() {
    ...
    listEl.innerHTML = taskChecklistBuilderItems.map((item, idx) => `
        <div class="checklist-builder-item">
            ...
            <span class="chk-item-text">${escapeHtml(item.text)}</span>
            <button type="button" class="btn-remove-chk" onclick="removeChecklistItemBuilder(${idx})" title="Eliminar paso">
                🗑️
            </button>
        </div>
    `).join('');
...
async function handleSaveTask(event) {
    ...
    if (isRecurring) {
        ...
        if (intervalSelect === 'custom_days') {
            if (!selectedRecurrenceDays || selectedRecurrenceDays.length === 0) {
                showNotification('⚠️ Por favor selecciona al menos un día de la semana para la repetición personalizada (ej. Martes y Viernes)', 'error');
                return;
            }
    ...
    if (!title) {
        showNotification('⚠️ El título de la tarea es requerido', 'error');
        return;
    }
    ...
    try {
        ...
        closeTaskModal();
        showNotification(`✅ Tarea ${isEdit ? 'actualizada' : 'creada'} exitosamente`, 'success');
        loadSupportTasks();
        loadSupportTaskStats();
    } catch (err) {
...
// =======================================================
// MODAL DE DETALLE DE TAREA Y CONFIRMACIÓN ESTILIZADA
// =======================================================

let currentConfirmCallback = null;
let currentCancelCallback = null;

function showConfirmDialog({ icon = '⚠️', title = '¿Confirmar Acción?', message = '¿Estás seguro?', confirmText = 'Confirmar', isDanger = true, onConfirm = null, onCancel = null }) {
    ...
    if (btnEl) {
        ...
        btnEl.style.background = isDanger ? '#DC2626' : '#008B8B';
...
function openTaskDetailModal(taskId) {
    ...
    if (headerTitle) {
        headerTitle.innerHTML = `📋 ${task.is_recurring ? '🔁 ' : ''}${escapeHtml(task.title)}`;
    }

    const priorityBadge = task.priority === 'urgent' ? '🔴 Urgente' : (task.priority === 'high' ? '🟠 Alta' : (task.priority === 'low' ? '🔵 Baja' : '🟡 Media'));
    const statusBadge = task.status === 'completed' ? '🟢 Completada' : (task.status === 'in-progress' ? '🔵 En Progreso' : '🟡 Pendiente');
    const checklist = Array.isArray(task.checklist) ? task.checklist : [];
    const metrics = task.checklistMetrics || { total: checklist.length, completed: checklist.filter(c => c.done).length };
    ...
    body.innerHTML = `
        ...
        ${task.description ? `
            <div style="background: #F9FAFB; border: 1px solid #E5E7EB; border-radius: 8px; padding: 12px 14px; margin-bottom: 16px; font-size: 0.92rem; color: #374151; line-height: 1.5;">
                ${escapeHtml(task.description).replace(/\n/g, '<br>')}
            </div>
        ` : ''}

        ${task.is_recurring ? `
            <div style="background: #F3E8FF; border: 1px solid #E9D5FF; border-radius: 8px; padding: 10px 14px; margin-bottom: 16px; font-size: 0.86rem; color: #6B21A8;">
                🔁 <strong>Rutina Recurrente Activa:</strong> Frecuencia <em>${formatRecurrenceLabel(task.recurrence_interval)}</em>. Al marcar el 100% de los pasos, se programará la siguiente fecha de forma automática.
            </div>
        ` : ''}
        ...
        <div style="margin-bottom: 20px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                <h4 style="font-size: 0.95rem; font-weight: 700; color: #1F2937; margin: 0;">
                    📝 Lista de Verificación (${metrics.completed}/${metrics.total})
                </h4>
                <span style="font-size: 0.82rem; font-weight: 700; color: ${progressPercent === 100 ? '#10B981' : '#0284C7'};">
            ...
            <div style="width: 100%; height: 8px; background: #E5E7EB; border-radius: 4px; overflow: hidden; margin-bottom: 12px;">
                <div style="width: ${progressPercent}%; height: 100%; background: ${progressPercent === 100 ? '#10B981' : '#008B8B'}; transition: width 0.3s ease;"></div>
            </div>

            ${checklist.length === 0 ? `
                <div style="text-align: center; padding: 16px; color: #9CA3AF; font-size: 0.88rem; background: #F9FAFB; border-radius: 8px;">
                    Esta tarea no tiene pasos de verificación definidos.
                </div>
            ` : `
                <div style="display: flex; flex-direction: column; gap: 8px;">
                    ${checklist.map((item, idx) => `
                        <label style="display: flex; align-items: flex-start; gap: 10px; padding: 10px 12px; background: ${item.done ? '#F0FDF4' : '#FFFFFF'}; border: 1px solid ${item.done ? '#BBF7D0' : '#E5E7EB'}; border-radius: 8px; cursor: pointer; transition: all 0.2s;">
                            <input type="checkbox" ${item.done ? 'checked' : ''} onchange="toggleChecklistItemInDetail(${task.id}, '${item.id || idx}', this.checked)" style="accent-color: #10B981; width: 18px; height: 18px; margin-top: 2px; cursor: pointer;">
                            <span style="font-size: 0.92rem; color: ${item.done ? '#15803D' : '#1F2937'}; ${item.done ? 'text-decoration: line-through;' : ''} flex: 1;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px solid #E5E7EB; padding-top: 16px;">
            <div style="display: flex; gap: 8px;">
                <button type="button" onclick="confirmDeleteTask(${task.id}, '${escapeHtml(task.title).replace(/'/g, "\\'")}')" style="padding: 8px 14px; background: #FEE2E2; color: #DC2626; border: 1px solid #FECACA; border-radius: 6px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px;">
                    🗑️ Eliminar Tarea
                </button>
                <button id="btnToggleStatusDetail-${task.id}" type="button" onclick="handleToggleStatusFromDetail(${task.id}, '${task.status}', this)" style="padding: 8px 14px; border-radius: 6px; font-size: 0.85rem; font-weight: 700; cursor: pointer; border: 1px solid #CBD5E1; background: ${task.status === 'completed' ? '#FEF3C7' : '#DCFCE7'}; color: ${task.status === 'completed' ? '#B45309' : '#15803D'}; display: inline-flex; align-items: center; gap: 6px; transition: all 0.2s ease;">
                    ${task.status === 'completed' ? '↩️ Reabrir Tarea' : '✅ Marcar Completada'}
                </button>
            </div>

            <div style="display: flex; gap: 10px;">
                <button type="button" onclick="editTaskFromDetail(${task.id})" style="padding: 8px 16px; background: #E0F2FE; color: #0284C7; border: 1px solid #BAE6FD; border-radius: 6px; font-weight: 600; cursor: pointer;">
                    ✏️ Editar
                </button>
...
async function toggleChecklistItemInDetail(taskId, itemId, done) {
    try {
        ...
        const idx = allSupportTasks.findIndex(t => t.id === taskId);
        if (idx !== -1) {
            allSupportTasks[idx] = updatedTask;
        }

        renderCalendar();
        loadSupportTaskStats();
        openTaskDetailModal(taskId); // Refrescar vista modal
    } catch (err) {
        showNotification(`❌ Error: ${err.message}`, 'error');
    }
}
...
function editTaskFromDetail(taskId) {
    const task = allSupportTasks.find(t => t.id === taskId);
...
async function changeTaskStatus(taskId, newStatus) {
    ...
    try {
        ...
        if (result.isRecurringCycleReset) {
            showNotification('🎉 ¡Ciclo completado! Tarea renovada y programada para el próximo vencimiento.', 'success');
        } else if (newStatus === 'completed') {
            showNotification('✅ Tarea marcada como completada.', 'success');
        } else {
            showNotification('↩️ Tarea reabierta.', 'info');
        }

        await loadSupportTasks();
        loadSupportTaskStats();
    } catch (err) {
        showNotification(`❌ Error: ${err.message}`, 'error');
    } finally {
...
function handleToggleStatusFromDetail(taskId, currentStatus, btnElement) {
    ...
    if (newStatus === 'completed' && task && task.checklistMetrics && !task.checklistMetrics.allDone && task.checklistMetrics.total > 0) {
        showConfirmDialog({
            icon: '⚠️',
            title: '¿Completar con pasos pendientes?',
            message: `Aún quedan <strong>${task.checklistMetrics.total - task.checklistMetrics.completed}</strong> pasos sin tildar en el checklist.<br>¿Deseas marcar la tarea como completada de todas formas?`,
...
function confirmDeleteTask(taskId, taskTitle) {
    showConfirmDialog({
        icon: '🗑️',
        title: '¿Eliminar Tarea de Soporte?',
...
async function executeDeleteTask(taskId) {
    try {
        ...
        if (!response.ok) {
            const errData = await response.json();
            throw new Error(errData.error || 'Error al eliminar tarea');
        }
...
// ==============================================
// EXPORTACIÓN A CSV DEL TABLERO DE SOPORTE
// ==============================================
function exportTasksToCSV() {
    if (!allSupportTasks || allSupportTasks.length === 0) {
    ...
    const lines = [
        `"Tablero de Tareas y Rutinas - Soporte Técnico"`,
        `"Fecha de Exportación";"${generatedAt}"`,
        `"Total Tareas";"${allSupportTasks.length}"`,
        ``,
        `"LISTADO DE TAREAS"`,
    ...
    link.setAttribute('download', `tablero_tareas_soporte_${todayStr}.csv`);
...
window.loadSupportTasks = loadSupportTasks;
window.loadSupportTaskStats = loadSupportTaskStats;
